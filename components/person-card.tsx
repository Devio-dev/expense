...
export function PersonCard({ person, setPeople }: PersonCardProps) {
  const router = useRouter()

  const deletePerson = (e: React.MouseEvent) => {
    e.stopPropagation()

    if (confirm(`¿Estás seguro de eliminar a ${person.name}?`)) {
      setPeople((prev) => prev.filter((p) => p.id !== person.id))
      localStorage.removeItem(`loanTracker_transactions_${person.id}`)
    }
  }
